...
- `npm run dev` works without `OPENAI_API_KEY`. However, `npm run build` requires a non-empty `OPENAI_API_KEY` to be set, because `app/api/ai/weekly-review/route.ts` and `app/api/ai/meal-estimate/route.ts` instantiate the OpenAI client at module load and the build's page-data collection imports those route modules. Any non-empty value lets the build complete; without it the build fails with "The OPENAI_API_KEY environment variable is missing or empty".
